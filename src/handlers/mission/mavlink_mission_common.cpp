...
void MavlinkMissionCommon::onVehicleObtained(Vehicle* vehicle)
{
    ...
    mission = new Mission(mission::mavlinkMission, tr("%1 mission").arg(vehicle->name), vehicle->id);
                                                 ...
                                                 const QVariant& vehicleId)
{
    ...
    mission->route()->reachItem(reached.seq);
}
                                               ...
                                               const QVariant& vehicleId)
{
    ...
    if (!mission->route()->isEmpty())
    {
        mission->route()->first()->position.set(
            Geodetic(utils::decodeLatLon(homePosition.latitude),
                     utils::decodeLatLon(homePosition.longitude),
                     utils::decodeAltitude(homePosition.altitude)));
    }

    // FIXME: vehicle's home position
}
